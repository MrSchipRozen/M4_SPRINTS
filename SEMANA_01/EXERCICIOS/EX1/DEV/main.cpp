...
int Converter(int c,int vMin,int vMax){
	c = c - vMin;
	vMax = vMax - vMin;
	int j = (c*100)/vMax;
	return j;
...
int guardarvetor(int pA, int f, int *vet, int t){
    int *pV = vet;
	pV[t];
	pV[pA] = f;
    return (pA + 1);
...
const char *direcaoMenorCaminho(int *vet,int *dir){
	const char *sentido[] = {"Esquerda", "Direita", "Tras", "Frente"};
	int l = 0;
	for (int i = 0; i < 4; i++) {
		if (vet[i] > *dir){
			*dir = vet[i];
	...
	// printf("O maior valor eh %d e o sentido eh %s",j,sentido[l]);
	return sentido[l];
...
int dirige(int *v,int maxv){
	...
	while(dirigindo == 1){
		for (int i = 0; i < 4; i++)	{
			int medida = lersens();/// .. Chame a função de de leitura da medida para a "Direita"
			medida = Converter(medida,0,830);
			posAtualVetor = guardarvetor(posAtualVetor,medida,vetorMov,maxVetor);
		}
